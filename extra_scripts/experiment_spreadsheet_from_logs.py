...
def parse_log(log_path: str, args) -> dict:
    ...
    if config:
        ...
        if args.parse_date_time:
            date_time = None
            try:
                date_time = parse_date_time(config[args.date_time_param],
                                            args.date_time_pattern,
                                            args.date_time_split_char)
            except:
                pass
            if not date_time:
                print(f'Unable to parse date/time')
                date_time = [None, None]
...
def parse_config_from_log(log_path: str) -> dict:
    ...
    config_start_split_on = "hydra_config.py: 126: "
    ...
    accuracies = {
        'train': {
            'string': 'train_accuracy_list_meter',
            'values': []
        },
        'test': {
            'string': 'test_accuracy_list_meter',
            'values': []
        }
    }
        ...
        for line in reader:
            ...
            if not config_finished:
                if config_start in line:
                    store_line = True
                    line = line.split(config_start_split_on)[1]
                if store_line:
                    config += line
    ...
    if config:
        ...
        try:
            ...
            print(f'Unable to parse dictionary')
            config = {}
        ...
        try:
            ...
            print(f'Unable to parse world size')
        try:
            ...
            print(f'Unable to parse final training loss')
            ...
            if partition_contents['values']:
                try:
                    ...
                    for top_string in ["top_1", "top_5"]:
                        acc = final_accuracy_string.split('value')[1].split(
                            top_string)
                        acc = acc[1].split('0: ')[1]
                        acc = acc.split('}')[0]
        ...
        print(f'No information parsed from log file')
        config = {}

    return config


def flatten(d: collections.abc.MutableMapping, parent_key: str = '',
            sep: str = '.'):
    items = []
    for k, v in d.items():
...
def parse_date_time(str_to_parse: str = None, pattern: str = None, split_char:
str = None):
...
def update_config_date_time(config: collections.OrderedDict,
                            date_time: typing.Union[list, tuple]):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--root_directory', type=str,
                        help='Directory containing experiment run or '
                             'subdirectories of runs')
    parser.add_argument('--output_directory', type=str,
                        default=os.getcwd(), help='Where to save output.')
    parser.add_argument('--output_name', type=str, default='experiments.txt',
                        help='Output filename')
    parser.add_argument('--parse_date_time', type=bool, default=True,
                        help='Parse date and time from config')
    parser.add_argument('--date_time_param', type=str, default='CHECKPOINT.DIR',
                        help='config param from whose value the date and time will be parsed')
    parser.add_argument('--date_time_pattern', type=str,
                        default='[0-9]{4}-[0-9][0-9]-[0-9][0-9]/[0-9][0-9]-[0-9][0-9]-[0-9][0-9]',
                        help='Regex pattern for date and time format')
    parser.add_argument('--date_time_split_char', type=str, default='/',
                        help='character to split date and time string into '
                             'separate strings')
    parser.add_argument('--log_file_name_pattern', type=str,
                        default='log.txt', help='pattern to match for log '
                                                'file names')
    parser.add_argument('--parse_checkpoint', type=bool, default=True,
                        help='Parse # training epochs from checkpoint file')
    parser.add_argument('--checkpoint_id_pattern', type=str,
                        default='_phase', help='pattern to match for '
                                                    'checkpoint file names')
    parser.add_argument('--checkpoint_extract_pattern', type=str,
                        default='phase([0-9]{1,4})\.torch',
                        help='pattern to extract epoch # from checkpoint file name')

    args = parser.parse_args()

    log_files = list(pathlib.Path(args.root_directory).rglob(
    ...
    for f in log_files:
        ...
        if args.parse_checkpoint:
            ...
            config['last_checkpoint_epoch'] = last_checkpoint
        configs_to_concat.append(config)
    df = pd.DataFrame(configs_to_concat)
    # Sort columns
    df = df.reindex(sorted(df.columns), axis=1)
    # Move specific columns to beginning
    prepend_columns = ['final_train_loss',
                       'final_test_loss',
