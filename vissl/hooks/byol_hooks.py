...
class BYOLHook(ClassyHook):
    ...
    def __init__(self, base_momentum: float, shuffle_batch: bool = True):
        ...
        # print('MOMENTUM: ', self.base_momentum)
    ...
    def target_ema(global_step, base_ema, max_steps):
        decay = BYOLHook.cosine_decay(global_step, max_steps, 1.)
    ...
    def _build_byol_target_network(self, task: tasks.ClassyTask) -> None:
        ...
        logging.info(
            "Building BYOL target network - rank %s %s", *get_machine_local_and_dist_rank()
        )

        # - same architecture but without the projector
        target_model_config = task.config['MODEL']
        target_model_config['HEAD']['PARAMS'] = target_model_config['HEAD']['PARAMS'][:2]
    ...
    def _update_momentum_coefficient(self, task: tasks.ClassyTask) -> None:
        ...
        coef = self.target_ema(current_iter, self.base_momentum, self.total_iters)
        # print('MOMENTUM: ', coef)
        self.momentum = coef
        self.inv_momentum = 1 - coef
    ...
    def _update_target_network(self, task: tasks.ClassyTask) -> None:
        ...
        newest encoder.
        """
        # Get the current iteration to update the momentum coefficient
    ...
    def on_forward(self, task: tasks.ClassyTask) -> None:
        ...
        if task.loss.target_network is None:
            ...
            logging.info("BYOL: Distributed setup, shuffling batches")
        else:
