...
but learn to be non-local over the course of training. This "inititialize
to be local, learn to be non-local" scheme has been shown to be extremely
data efficient, and outperform DeiTs. From unpublished work by D'Ascoli,
Touvron, Leavitt, Morcos, Biroli, and Sagun. Implementation modified from
...
class GPSA(nn.Module):
    ...
    def __init__(
        ...
        # in the paper
        use_local_init=True,  # Local initialization
    ):
        super().__init__()
...
class SelfAttention(nn.Module):
    ...
    def __init__(
        ...
        qk_scale=None,  # Scale query and key
        attention_dropout_rate=0.0,  # Dropout on attention
        proj_dropout_rate=0.0,  # Dropout on linear projection after attention
    ):
        super().__init__()
        self.num_heads = num_heads
        head_dim = embed_dim // num_heads
