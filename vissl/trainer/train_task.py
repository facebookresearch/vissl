...
import torch
from classy_vision.fb.optim.zero import ZeRO
...
class SelfSupervisionTask(ClassificationTask):
    ...
    def set_amp_args(self):
        ...
        if self.config.MODEL.AMP_PARAMS.USE_AMP:
            ...
            # Check Apex or CUDA availability
            ...
            if self.amp_type == AmpType.pytorch and self.device != torch.device("cuda"):
                raise RuntimeError(
                    "Pytorch AMP is only available for CUDA. Can't use mixed precision"
                )
