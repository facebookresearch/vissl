...
for root, dirs, files in os.walk("logs/test_linear"):
    if len(dirs) == 0:
        split = []
        head=root
        for i in range(4):
...
for cm in extract_features:
    for model in extract_features[cm]:
        ...
        os.makedirs(f"plots/data/{cm}/{model}", exist_ok=True)
        np.save(f"plots/data/{cm}/{model}/feats.npy", feats)
...
for cm in test_linear:
    for model in test_linear[cm]:
        ...
        df.to_csv(f"plots/data/{cm}/{model}/test_linear.csv")
...
for cm in nearest_neighbor:
    for model in nearest_neighbor[cm]:
        ...
        df.to_csv(f"plots/data/{cm}/{model}/nearest_neighbor.csv")
